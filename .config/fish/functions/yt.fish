...
function yt
    ...
    function __yt_download --no-scope-shadowing
        ...
        if test "$type" = mp3
            yt-dlp --no-config --cookies-from-browser firefox -x --audio-format mp3 --audio-quality 0 --no-playlist --no-video -o "$out/$name_template" "$url"
        else
            yt-dlp --no-config --cookies-from-browser firefox -f "bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=720]+bestaudio/best[height<=720]/best" --merge-output-format mp4 --no-playlist -o "$out/$name_template" "$url"
        end
    end
...
function ytl
    ...
    function __ytl_download --no-scope-shadowing
        ...
        if test "$dl_type" = mp3
            yt-dlp --no-config $cookies_arg -x --audio-format mp3 --audio-quality 0 --no-playlist --no-video -o "$out/$name_template" "$url"
        else
