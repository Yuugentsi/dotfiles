...
case "${1:-}" in
    ...
    # ----- mp3 player (ALT + M) -----
    # bind_exec("ALT + M", "$HOME/.config/hypr/scripts/utils.sh mp3")
    mp3)
        MUSIC_DIR="${HOME}/0/music"
        CACHE_DIR="$HOME/.cache/scripts/audio"
        CACHE_FILE="$CACHE_DIR/cache.txt"
        CACHE_MTIME="$CACHE_DIR/cache-mtime.txt"
        PLAYED_FILE="$CACHE_DIR/played.txt"
        SOCKET="$HOME/.config/mpv-socket"

        _mp3_get_mtime() {
            fd -t f -i ".mp3" "$MUSIC_DIR" | xargs -r stat --format="%Y %s %n" 2>/dev/null | sort | md5sum
        }

        _mp3_build_cache() {
            mkdir -p "$CACHE_DIR"
            fd -t f -i ".mp3" "$MUSIC_DIR" | sort > "$CACHE_FILE"
            _mp3_get_mtime > "$CACHE_MTIME"
        }

        _mp3_refresh_cache() {
            if [[ ! -f "$CACHE_FILE" ]] || [[ ! -f "$CACHE_MTIME" ]]; then
                _mp3_build_cache
                return
            fi
            local current cached
            current=$(_mp3_get_mtime)
            cached=$(cat "$CACHE_MTIME")
            [[ "$current" != "$cached" ]] && _mp3_build_cache
        }
        ...
        tmp_all=$(mktemp)
        tmp_unplayed=$(mktemp)
        cp "$CACHE_FILE" "$tmp_all"
        touch "$PLAYED_FILE"

        grep -Fxv -f "$PLAYED_FILE" "$tmp_all" > "$tmp_unplayed" 2>/dev/null
        ...
        choice=$(shuf -n1 "$tmp_unplayed")

        if [[ -n "$choice" ]]; then
            echo "$choice" >> "$PLAYED_FILE"

            tmp_played=$(mktemp)
            awk '!visited[$0]++' "$PLAYED_FILE" > "$tmp_played" 2>/dev/null
            mv "$tmp_played" "$PLAYED_FILE"

            if pgrep -x mpv >/dev/null; then
                { echo "loadfile \"$choice\" insert-at 0"; echo "playlist-play-index 0"; echo "playlist-move 1 999"; } | socat - "$SOCKET" 2>/dev/null
            else
                mpv --no-video --no-resume-playback "$choice" >/dev/null 2>&1 &
            fi

            song_name=$(basename "$choice" .mp3 | tr '"' "'")
            hyprctl dismissnotify 1
            hyprctl notify 1 4000 "rgb(f8b4c8)" "fontsize:24 $song_name"
        fi
    ...
    files)
        ...
        ;;
    *)
        echo "Usage: $0 {toggle|play|z|note|files|workspace}"
        exit 1
        ;;
esac
